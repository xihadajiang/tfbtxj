    ...
    def run(self):
        ...
        while True:
            try:
                ...
                logging.debug('line[%s] Dispatch Socket read error of %s: %s' % (lineno(),addr_in, str(e)))
                break
            if not data:
                logging.debug('无数据break')
                break
...
class Proxy(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            self.sock_in.close()
            logging.debug('Remote error: %s' % str(e))
            return
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.debug('Strange error creating socket: %s' % str(e))
        sys.exit(1)
    try:
        ...
        logging.debug('socket bind error : %s' % str(e))
        sock_master.close()
        sys.exit(1)
        
    logging.debug('Listening at %s:%d ...' % (local_addr, local_port))
    i = 0
    chkerlist = []
    while i < THREAD_NM:
